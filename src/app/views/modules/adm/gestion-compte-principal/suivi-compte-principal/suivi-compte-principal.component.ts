...
export class SuiviComptePrincipalComponent extends Translatable implements OnInit {
    ...
    header = [
        ...
        {"nomColonne" :  this.__('suivi_compte.montant'),"colonneTable" : "montant","table" : "releve_des_comptes", "align": "right"},
    ...
    afficheSolde(){
      const soldeLocal = localStorage.getItem(environment.soldeSuiviCompte);
      const soldeCarteLocal = localStorage.getItem(environment.soldeCarteSuiviCompte);
    ...
    filtreTableau() {
        ...
        const filtreParMulti =  filtre_search + filtreDate + "&__order__=desc,date_transaction";
