...
export class ActionService {
    ...
    ajoutAction(): Observable<any> {
        
        return this.httpService.get<any>(environment.appendroute).pipe(
            tap(response => {
    ...
    genererRoute(): Observable<any> {
        
        return this.httpService.get<any>(environment.generateroute).pipe(
            tap(response => {
                if (response['code'] === 200) {
                    console.log("response XHR", response)
                }
